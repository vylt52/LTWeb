										...
										<tbody>
											...
											<c:forEach items="${users}" var="user" varStatus="loop">
												<tr class="${user.isActive == false ? 'bg-light' : ''}">
													...
													<td><c:choose>
															<c:when test="${user.isAdmin == true}"></c:when>
															<c:otherwise>
																<div class="btn-group" role="group">
																	<button class="btn btn-primary ms-2 rounded-2"
																		onclick="GetUserId('${user.id}')">Sửa</button>
																</div>
															</c:otherwise>
														</c:choose></td>
												</tr>
							...
							<ul class="pagination justify-content-center">
								...
								<c:if test="${currenPage > 1}">
									<li class="page-item text-secondary"><a class="page-link"
										href="datauser?page=${currenPage - 1}" aria-disabled="true">
								...
								<c:forEach varStatus="i" begin="1" end="${maxPage}">
									<li
										...
										<a class="page-link"
										href="<c:url value='datauser?page=${i.index}'/>">${i.index}</a>
									</li>
								</c:forEach>
